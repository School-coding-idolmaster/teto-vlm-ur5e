from src import grounding as grounding_package
...
def test_grounding_package_root_keeps_empty_export_policy():
    assert grounding_package.__all__ == []


def test_grounding_result_current_import_exports_public_api():
    names = [
        "CONTRACT_VERSION",
    ...
    for name in names:
        assert hasattr(new_result, name)


def test_vlm_grounding_adapter_current_import_exports_public_api():
    names = [
        "CONTRACT_VERSION",
...
def test_forbidden_fields_helper_preserves_current_imports_and_detection():
